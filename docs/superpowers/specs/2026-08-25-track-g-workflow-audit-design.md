...
after=..., project_id=None, correlation_id=...)`, called explicitly at
each mutation site — matching the existing house style of
`assert_company_access`/`assert_evidence_belongs_to_company`, not a
hidden SQLAlchemy event hook. `correlation_id` is generated once per
HTTP request (middleware) and threaded through.
...
- `id`, `entity_type`, `entity_id`, `company_id`, `project_id` (nullable),
  `module` (str, e.g. `"ap"`, `"construction"`), `requested_by` (FK),
  `assigned_to` (FK, nullable) / `assigned_role` (str, nullable — one of
  the two is set), `status` (`PENDING`/`APPROVED`/`REJECTED`/
  `CANCELLED`), `priority` (`LOW`/`NORMAL`/`HIGH`), `amount` (Decimal,
  nullable — for filtering/display only, never authoritative), `comment`
  (str, nullable, set on decision), `decided_by` (FK, nullable),
...
A `requires_third_role: bool` flag on the approval-type registration
...
brief's two most concrete SoD-sensitive flows), not retrofitted to every
domain in this task.
