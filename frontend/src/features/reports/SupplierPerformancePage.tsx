...
const COLUMNS: TableColumn<SupplierPerformanceRow>[] = [
  ...
  { key: 'onTimeDeliveryRate', header: 'Entrega a tiempo', render: (row) => formatRate(row.onTimeDeliveryRate, row.onTimeDeliverySampleSize) },
  { key: 'threeWayMatchCleanRate', header: 'Three-way match sin excepción', render: (row) => formatRate(row.threeWayMatchCleanRate, row.threeWayMatchSampleSize) },
  { key: 'priceVariancePct', header: 'Variación de precio', render: (row) => formatRate(row.priceVariancePct, row.priceVarianceSampleSize) },
]
...
export function SupplierPerformancePage() {
  ...
  if (!activeCompanyId) return <EmptyState icon="users" title="Configura una compañía primero" description="No hay compañías registradas todavía." />
    ...
    <div>
      <header className="nx-page__header">
        ...
        <ReportExportButtons
          ...
          basename="desempeno-proveedores"
          exportFile={(format) => reportingService.exportSupplierPerformance(activeCompanyId, format)}
        />
      </header>
      {reportQuery.isLoading ? <LoadingState label="Cargando desempeño de proveedores…" /> : reportQuery.isError ? (
        <ErrorState onRetry={() => reportQuery.refetch()} />
      ) : (
        <Card><Table columns={COLUMNS} rows={rows} getRowKey={(row) => row.supplierId} emptyMessage="Sin proveedores registrados todavía." /></Card>
      )}
    </div>
  )
}
