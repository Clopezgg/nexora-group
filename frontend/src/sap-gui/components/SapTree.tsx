/**
 * NEXORA SAP GUI Classic — Tree Primitive
 * 
 * Árbol estilo SAP Easy Access.
 * - Expandir/colapsar por grupo
 * - Navegación por teclado completa (flechas, Home, End, Enter)
 * - Búsqueda con filtrado
 * - Typeahead por letra inicial
 * - Estado de foco visible
 * - Expansión controlada (no se reabre forzadamente)
 */
...
export const SapTree = ({ groups: allGroups, onNavigate, variant = 'sidebar' }: SapTreeProps) => {
  ...
  const [visibleGroups] = useState(allGroups);
  ...
  return (
    <nav
      ref={treeRef}
      className={`nx-sap-tree ${isDrawer ? 'nx-sap-tree--drawer' : ''}`}
    ...
    >
      ...
      {filteredGroups.length === 0 ? (
        <p className="nx-sap-tree__empty" role="status">Sin módulos coincidentes.</p>
      ) : null}
      ...
      {filteredGroups.map((group) => {
        ...
        return (
          <div key={group.key} className="nx-sap-tree__group nx-sap-tree__group--collapsible" role="none">
            <div
              className="nx-sap-tree__group-label"
              role="treeitem"
              aria-expanded={open}
              aria-selected="false"
              aria-level={1}
            ...
            {open ? (
              <ul className="nx-sap-tree__list" role="group" aria-label={group.label}>
