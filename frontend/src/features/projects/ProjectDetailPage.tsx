...
function ProjectEditForm({ project, customers, users, costCenters, onUpdated }: {
  project: Project
  ...
  const updateMutation = useMutation({
    mutationFn: () => projectService.update(project.id, {
      name: form.name.trim(), code: form.code.trim() || undefined, customerId: form.customerId || null,
      managerUserId: form.managerUserId || null, currencyCode: form.currencyCode,
      costCenterId: form.costCenterId || null, plannedStart: form.plannedStart || null,
      plannedEnd: form.plannedEnd || null, description: form.description.trim() || null,
      addressLine1: form.addressLine1.trim() || null, addressLine2: form.addressLine2.trim() || null,
      city: form.city.trim() || null, stateDepartment: form.stateDepartment.trim() || null,
      country: form.country.trim() || null, locationReference: form.locationReference.trim() || null,
    }),
    onSuccess: onUpdated,
  })
  ...
  return (
    <form onSubmit={(event) => { event.preventDefault(); updateMutation.mutate() }}>
      ...
      <Select label="Cliente" value={form.customerId} onChange={(event) => setForm({ ...form, customerId: event.target.value })}>
        <option value="">Sin cliente asignado</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.legalName}</option>)}
      </Select>
      <Select label="Responsable" value={form.managerUserId} onChange={(event) => setForm({ ...form, managerUserId: event.target.value })}>
        <option value="">Sin responsable asignado</option>{users.map((user) => <option key={user.id} value={user.id}>{user.fullName}</option>)}
      </Select>
      <Select label="Centro de costo" value={form.costCenterId} onChange={(event) => setForm({ ...form, costCenterId: event.target.value })}>
        <option value="">Sin centro de costo</option>{costCenters.map((item) => <option key={item.id} value={item.id}>{item.code} · {item.name}</option>)}
      </Select>
...
export function ProjectDetailPage() {
  ...
  const projectQuery = useQuery({ queryKey: ['project', projectId], queryFn: () => projectService.get(projectId as string), enabled: Boolean(projectId) })
  const financialQuery = useQuery({ queryKey: ['project', projectId, 'financial-summary'], queryFn: () => projectService.getFinancialSummary(projectId as string), enabled: Boolean(projectId) })
  const customersQuery = useQuery({ queryKey: ['crm', 'customers', activeCompanyId], queryFn: () => crmService.listCustomers(activeCompanyId as string), enabled: Boolean(activeCompanyId) })
  ...
  const selectedCustomer = useMemo(() => (customersQuery.data ?? []).find((customer) => customer.id === project?.customerId) ?? null, [customersQuery.data, project?.customerId])

  if (projectQuery.isLoading) return <LoadingState label="Cargando proyecto…" />
  if (projectQuery.isError || !project) return <ErrorState description="No se pudo cargar el proyecto." onRetry={() => projectQuery.refetch()} />
  if (activeCompanyId && project.companyId !== activeCompanyId) return <EmptyState title="Proyecto fuera de la empresa activa" description="Cambia la empresa activa para abrir este proyecto." />

  const summary = financialQuery.data
  const currency = summary?.currencyCode ?? project.currencyCode ?? 'HNL'
  const managerName = (usersQuery.data ?? []).find((u) => u.id === project.managerUserId)?.fullName ?? project.manager ?? 'Sin responsable'
  const locationText = [project.addressLine1, project.city, project.stateDepartment, project.country].filter(Boolean).join(', ') || 'Sin ubicación registrada'
  const planText = project.plannedStart && project.plannedEnd ? `${project.plannedStart} → ${project.plannedEnd}` : '—'

  const statusCard = <ProjectStatusCard project={project} onUpdated={(updated) => { if (context.activeProjectId === updated.id) setActiveProject(updated.id) }} />
  ...
  const resumenTab = <>
    ...
    {summary ? <Card title="Resumen comercial y rentabilidad">
      ...
      {summary.contractValue === null ? <p className="nx-field__hint">El valor comercial aparecerá cuando exista un contrato de venta asociado; no se confunde con contratos de ejecución.</p> : null}
    </Card> : null}
  </>
  ...
  const finanzasTab = summary ? <>
    <Card title="Costos y ejecución">
      <div className="nx-home__grid">
        ...
        {summary.available !== null ? <StatCard label="Disponible de presupuesto" value={money(summary.available, currency)} /> : <>
      ...
      {summary.available === null ? <p className="nx-field__hint">Este proyecto no tiene un presupuesto BASELINE. La cifra mostrada es exposición real, no un disponible negativo. <Link to="/proyectos/presupuestos">Configurar WBS y presupuesto</Link>.</p> : null}
    </Card>
    ...
    <Card title="Earned Value">
      <div className="nx-home__grid">
        <StatCard label="BAC" value={money(summary.bac, currency)} /><StatCard label="PV" value={money(summary.pv, currency)} />
        <StatCard label="EV" value={money(summary.ev, currency)} /><StatCard label="AC" value={money(summary.ac, currency)} />
        <StatCard label="CPI" value={summary.cpi ?? '—'} /><StatCard label="SPI" value={summary.spi ?? '—'} />
        <StatCard label="ETC" value={money(summary.etc, currency)} /><StatCard label="EAC" value={money(summary.eac, currency)} /><StatCard label="VAC" value={money(summary.vac, currency)} />
      </div>
    </Card>
  ...
  const fichaTab = <>
    ...
    <Card title="Ficha del proyecto">
      <ProjectEditForm key={project.id} project={project} customers={customersQuery.data ?? []} users={usersQuery.data ?? []} costCenters={costCentersQuery.data ?? []} onUpdated={(updated) => { invalidate(); if (context.activeProjectId === updated.id) setActiveProject(updated.id) }} />
    </Card>
  </>

  const accionesTab = <>
    <Card title="Qué hacer ahora">
      <p className="nx-field__hint">Trabaja por proceso, no por módulos técnicos. Para obligaciones de contratistas usa primero la pestaña Contratos: desde allí preparas la obligación y pagas la próxima cuota con contexto contractual.</p>
    ...
    <Card title="Excepciones financieras">
      <p className="nx-field__hint">Cuentas por pagar se usa para facturas u obligaciones sueltas y para aprobaciones pendientes. Los pagos contractuales normales deben iniciarse desde Contratos.</p>
      <div className="nx-treasury__actions">
    ...
    <div className="nx-object-page">
      <header className="nx-object-header">
        <div className="nx-object-header__top">
          <div><p className="nx-object-header__eyebrow">Proyecto · {project.code ?? 'sin código'}</p><h1 className="nx-dashboard__title">{project.name}</h1></div>
          <div className="nx-object-header__actions">
            <Badge>{projectStatusLabel(project.status)}</Badge>
            {context.activeProjectId !== project.id ? <Button variant="secondary" onClick={() => setActiveProject(project.id)}>Seleccionar como contexto</Button> : <Badge tone="info">Contexto activo</Badge>}
          </div>
        </div>
        <dl className="nx-object-header__facts">
          ...
          {summary ? <><div><dt>Costo contratado de ejecución</dt><dd>{money(summary.executionContractValue, currency)}</dd></div><div><dt>Saldo contractual de ejecución</dt><dd>{money(summary.executionContractBalance, currency)}</dd></div></> : null}
        </dl>
      </header>
      ...
      <Tabs items={[
        ...
        { key: 'contratos', label: 'Contratos y pagos', content: <ProjectContractsTab companyId={project.companyId} projectId={project.id} /> },
