...
export function ContractPaymentPlanModal({ contract, currencyCode, onClose }: {
  ...
  const columns: TableColumn<ContractInstallment>[] = [
    ...
    { key: 'balanceBefore', header: 'Saldo contractual anterior', numeric: true, render: (r) => formatMoney(r.runningContractBalanceBefore, currency) },
    { key: 'scheduled', header: 'Programado de la cuota', numeric: true, render: (r) => formatMoney(r.scheduledAmount, currency) },
    { key: 'retention', header: 'Retención de la cuota', numeric: true, render: (r) => r.retentionAmount !== '0' && r.retentionAmount !== '0.00' ? formatMoney(r.retentionAmount, currency) : '—' },
    { key: 'netDue', header: 'Obligación neta de la cuota', numeric: true, render: (r) => formatMoney(r.netDue, currency) },
    { key: 'paid', header: 'Pagado en la cuota', numeric: true, render: (r) => formatMoney(r.paid, currency) },
    { key: 'remaining', header: 'Pendiente de la cuota', numeric: true, render: (r) => formatMoney(r.remaining, currency) },
    { key: 'balanceAfter', header: 'Saldo contractual posterior', numeric: true, render: (r) => formatMoney(r.contractBalanceAfter, currency) },
