...
| NXR-REQ-0093 | Reporting (por dominio) | ✅·➖·✅·✅·✅·✅·➖·✅·⬜ | IN_PROGRESS | Plan "reports-search-analytics", Task 2: alcance deliberadamente acotado por la Ruling de `docs/superpowers/specs/2026-08-25-reports-search-analytics-design.md` -- construye SOLO Trial Balance + Budget vs Actual, reusando los servicios ya confiables `treasury_service.account_balance` (no `accounting_service` -- ese módulo no existe, el balance vive en Treasury) y `budget_service.compute_summary` sin recalcular nada en paralelo; `reporting_service.trial_balance`/`budget_vs_actual` reales (`app/services/reporting_service.py`), API `GET /api/reports/trial-balance`/`GET /api/reports/budget-vs-actual` con `assert_company_access` (budget-vs-actual resuelve el proyecto primero para obtener su `company_id`, mismo patrón que toda ruta project-scoped), permisos nuevos `reports.trial_balance`/`reports.budget_vs_actual` (Finance Manager/Accountant/Auditor para trial balance; Finance Manager/Project Manager/Project Controller/Auditor para budget vs actual); convención de signo verificada contra el código real: `account_balance` siempre devuelve débito-crédito sin reclasificar por `account_type`, así que un balance positivo va a la columna débito y uno negativo a la crédito tal cual (correcto para un Trial Balance, a diferencia de un Balance Sheet) -- test de regresión real con una cuenta REVENUE (normalmente acreedora) confirmando que aparece en la columna crédito; RED/GREEN evidence real (`backend/tests/test_reporting.py`, 5 tests: cuadre débito=crédito sobre un asiento real, regresión de signo, dos tests de aislamiento de company); `TrialBalancePage.tsx`/`BudgetVsActualPage.tsx` reales montadas como tabs de `ReportsPage.tsx` bajo `/control/reportes` (entrada de nav ya reservada, no se inventó ruta nueva, mismo criterio que `/control/auditoria`/`/inicio/aprobaciones`). Explícitamente FUERA de alcance de esta fase (no construido, ni parcialmente): Balance Sheet, P&L, Cash Flow, reportes de Treasury/Procurement, Earned Value (CPI/SPI/EAC/VAC ya viven en `ForecastResponse`, no se duplican aquí) -- columna DB marcada `➖` porque ningún reporte de esta fase crea tabla nueva (son lecturas puras sobre datos ya existentes); columna Audit marcada `➖` por el mismo criterio que Notifications (NXR-REQ-0091): leer un reporte no es en sí un evento de negocio auditable; falta E2E. Ver `task-2-report.md` |
