...
- El menú SAP de móvil mide 40 px y el command row mide 88 px para alojar dos filas reales (toolbar 40 px y búsqueda 48 px). La prueba compara rectángulos DOM y exige que menú, command row y Topbar no se superpongan en 360, 390 y 768 px.
- La matriz visual y las pruebas de accesibilidad deben recorrer navegaciones y tamaños reales, sin sustituir fallos con exclusiones.
- El recorrido de reversión AP debe obtener una capacidad Protected Edit nueva antes de comprobar que un segundo reversal recibe el conflicto de negocio `409`; un `428` sin capacidad válida es la protección correcta y no debe eliminarse.
